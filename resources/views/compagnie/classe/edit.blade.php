                    ...
                    <div class="col-12">
                        <div class="card shadow-sm">
                            {{-- <div class="card-header bg-light">
                                <h5 class="mb-0">
                                    <i class="fas fa-layer-group me-2"></i>Informations de la classe
                                </h5>
                            </div> --}}
                        ...
                        {{-- <form action="{{ route('classe.update', $classe->id) }}" method="POST" novalidate> --}}

<form id="classeForm" action="{{ route('classe.update', $classe->id) }}" method="POST" novalidate>
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <textarea class="form-control @error('description') is-invalid @enderror" 
                                              id="description" name="description" rows="8">{{ old('description', $classe->description) }}</textarea>
                                    @error('description')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-12">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" 
                                               id="est_actif" name="est_actif" value="1" 
                                               {{ old('est_actif', $classe->est_actif) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="est_actif">
                                            {{ $classe->est_actif ? 'Activé' : 'Désactivé' }}
                                        </label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex justify-content-end pt-3 border-top mt-4">
                                <button type="submit" id="submitBtn" class="btn btn-warning d-flex align-items-center">
...
<style>
    ...
    #loader {
        /* Plus besoin d’absolute ici si on utilise flex */
    }
...
<script>
    $(document).ready(function() {
        $('#classeForm').on('submit', function() {
            ...
            // Afficher le spinner et changer le texte
